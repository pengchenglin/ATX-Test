...
class BasePage(object):
    ...
    def local_install(cls, apk_path, clear=False, uninstall=True):
        ...
        if clear:
            ...
            if packagename in internalapp:
                ...
                log.i('xiaoyingapp下没有%s的配置信息' % packagename)
        else:
            pass
        if uninstall:
        ...
        if cls.d.device_info['brand'] == 'vivo':
            ...
            cls.watch_device('允许|继续安装|允许安装|始终允许|安装|重新安装|同意|确定')
    ...
    def watch_device(cls, keyword):
        ...
        cls.d.watcher.start()
        time.sleep(2)
        # with cls.d.watch_context() as ctx:
        #     for i in keyword.split("|"):
        #         ctx.when("%s" % i).click()
        # ctx.start()
    ...
    def swipe_up(self, element=None, steps=30):
        """
        swipe up
    ...
    def swipe_down(self, element=None, steps=30):
        """
        swipe down
        :param element: UI element, if None while swipe window of phone
        :param steps: steps of swipe for Android, The lower the faster
    ...
    def swipe_left(self, element=None, steps=30):
        """
        swipe left
        :param element: UI element, if None while swipe window of phone
        :param steps: steps of swipe for Android, The lower the faster
    ...
    def swipe_right(self, element=None, steps=30):
        """
        swipe right
        :param element: UI element, if None while swipe window of phone
        :param steps: steps of swipe for Android, The lower the faster
    ...
    def _find_element_by_swipe(self, direction, value, element=None, steps=30, max_swipe=6):
        """
    ...
    def find_element_by_swipe_up(self, value, element=None, steps=30, max_swipe=40):
        return self._find_element_by_swipe('up', value,
                                           element=element, steps=steps, max_swipe=max_swipe)

    def find_element_by_swipe_down(self, value, element=None, steps=30, max_swipe=40):
        return self._find_element_by_swipe('down', value,
                                           element=element, steps=steps, max_swipe=max_swipe)

    def find_element_by_swipe_left(self, value, element=None, steps=30, max_swipe=40):
        return self._find_element_by_swipe('left', value,
                                           element=element, steps=steps, max_swipe=max_swipe)

    def find_element_by_swipe_right(self, value, element=None, steps=30, max_swipe=40):
